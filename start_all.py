...
def main():
    ...
    # Run as module (-m) from ROOT so Python doesn't prepend /app/api to sys.path,
    # which would cause app.py to shadow the 'api' package on imports.
    ...
    try:
        while True:
            ...
            if monitor_proc is None and CHECKPOINT.exists():
                ...
                # Run as module (-m) from ROOT so Python doesn't prepend /app/monitor
                # to sys.path — otherwise monitor.py shadows the 'monitor' package
                # and `from monitor.feature_extractor import ...` breaks.
